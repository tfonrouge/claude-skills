...
description: >
  ...
  SUBSYSTEM MODE — deep structural change to a codebase component
  (type system, VM refactor, GC rewrite, new IR/backend).
  Triggers: "refactor the VM", "rewrite the GC", "add type checking",
  or multi-file structural changes to a compiler/runtime.

  FEATURE MODE — self-contained addition plugging into existing architecture
  (new opcode, debug protocol, builtin, driver).
  Triggers: "add DAP support", "implement trait syntax", "new driver",
  or extending without restructuring.

  PATCH MODE — targeted fix with minimal blast radius
  (bug fix, perf optimization, cleanup).
  Triggers: "fix the GC pause", "optimize dispatch", "clean up dead code",
  or single-concern changes.

  Artifacts: (SUBSYSTEM) for structural changes, (FEATURE) for additions,
  (PATCH) for fixes.
---
